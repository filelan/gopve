...
func TestVirtualMachineSnapshot(t *testing.T) {
	...
	t.Run("List", func(t *testing.T) {
		...
		expectedSnapshots := []types.Snapshot{
			vm.NewSnapshot(virtualMachine, "first", "First snapshot", time.Unix(1609372800, 0).In(loc), true, ""),
			vm.NewSnapshot(virtualMachine, "second", "Second snapshot", time.Unix(1609416000, 0).In(loc), false, "first"),
	...
	t.Run("Get", func(t *testing.T) {
		...
		expectedSnapshot := vm.NewSnapshot(virtualMachine, "first", "First snapshot", time.Unix(1609372800, 0).In(loc), true, "")
